...
function doRefresh(selectorList, updateViewstate) {
    ...
    if ($('.ui-dialog:visible').length == 0 && latch) {   //Don't do a page update if a dialog is open. That could get messy.
        ...
        refreshPage(selectorList, updateViewstate, data, false);
...
/********************************** Event and cal functions **********************/

function popup(url, clientID) {
    if (url != null && url != "") {
        window.open(url + "?client=" + clientID, "popup");
    }
    return true;
}
...
function createAppt(date, userid, username) {
    if (!username) username = "";
    if (!userid) userid = -1;
    if (userid == -1) username = "Current user";
    ...
    if (hr < 12) ap = 'AM';
    if (hr > 12) hr = hr - 12;
    createDlg('addappt.aspx?userid=' + userid + '&selDate=' + (date.getMonth() + 1) + "/" + date.getDate() + '/' + date.getFullYear() + ' ' + hr + ":" + min + ' ' + ap, 'Add Appointment: ' + username, 450, 750, true);
}

function showevent(event, jsEvent, view) {
    var apptid = "#appt_" + event.id;
    if ($(apptid).length) {
        var item = $(this).children(":first").children(":first").children(":first");
        $(apptid).parent().slideDown();
        item.effect("transfer", { to: $(apptid) }, 1000, function() { });
    } else {
       apptPopup(event.id);
    }
}

function apptPopup(apptid){
     createDlg('AppointmentFrame.aspx?apptid=' + apptid, 'Appointment Details', 400, 495, true);
}

function editClient(clientID) { 
    createDlg('EditDemographics.aspx?id='+clientID, 'Edit Client', 505, 800, true);
}

var url = "";
function deptReturn(msg) {
    $('button, input:submit, input:button, input:reset').button();
    $('input[type=text],input[type=password],textarea,select').textbox();
    $('input:checkbox').checkbox();
    $('input:radio').radio();
    ...
    $('.ddProgram').hide();
    if ($(".ddDepartment").val() == "NULL") {
        $("#searcher").slideUp();
        $(".ddProgram").slideUp();
    } else if ($(".ddProgram option").size() <= 1) {
        $(".ddProgram").slideUp();
        $("#searcher").slideDown();
    } else {
        $('.ddProgram').slideDown();
        ...
        if ($(".ddProgram option").size() == 2) {
            $(".ddProgram")[0].selectedIndex = 1; ;
            $("#searcher").slideDown();
        }
    }
    refetch();
...
function refetch() {
    //if ($(".cal1:visible")) {
        var newurl = BuildUrl();
        $('.cal1').fullCalendar('removeEventSource', $('.cal1').fullCalendar('clientSources')[1].url);
        $('.cal1').fullCalendar('addEventSource', newurl);
        //$('.cal1').fullCalendar('removeEvents');
        //$('.cal1').fullCalendar('refetchEvents');

        if ($('.cal1').fullCalendar('getView').name == 'month') {
            $('.cal1').fullCalendar('changeView', 'agendaWeek');
            $('.cal1').fullCalendar('changeView', 'month')
        }
   // }
}
...
function BuildUrl() {
    if (url == "")
        url = $('.cal1').fullCalendar('clientSources')[1].url;
    var urladd = url;
    if ($(".ddDepartment").val() != "-1")
        urladd += '&dept=' + $(".ddDepartment").val();
    if ($(".ddDepartment").val() != "-1" && $(".ddProgram").val() != "-1")
        urladd += '&pgm=' + $(".ddProgram").val();
    if (!$("#showCanceled").is(':checked'))
        urladd += '&showCanceled=true'
    //if (!$("#showEnded").is(':checked'))
    //    urladd += '&showEnded=true'
    return urladd;
}

function PrintReport() {
    var mywindow = window.open('', 'PrintWindow', 'width=800,height=800');
    var html = '<html><head> <link rel="stylesheet" type="text/css" href="js/printstyle.css"> \
    <link id="ui-theme"  rel="stylesheet" type="text/css" href="~/res/BaseClasses/Scripts.aspx?f=jQueryLibrary/aristo.aristo1.css" /> \
    <script type="text/javascript" src="js/SiteFunctions.js"></script></head><body onload="formatForPrinting();">' + $('<div />').append($('.DTIReport').clone()).html() + '</div></body></html>';
