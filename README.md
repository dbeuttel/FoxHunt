...
- **v1.2 — National emergency-services map + scanner radio** (scheduled). New `/EmergencyMap.aspx` on the same Leaflet foundation. National scope — municipal open-data CAD feeds from ~15 US cities (Seattle, SF, Boston, NYC, Chicago, Raleigh, Charlotte, DC, Austin, Portland, Minneapolis, Denver, Phoenix, Oakland, Durham if published) + live police/EMS **helicopter tracks** via OpenSky/adsb.fi + **inline scanner audio**: incident popups embed an audio player playing the last 2–3 call recordings from the relevant OpenMHz talkgroup, plus an optional activity-heatmap layer showing where scanner traffic is spiking. Encrypted talkgroups skipped by design. **No ground-vehicle GPS tracking** — that data isn't publicly available; this is an incident-awareness + radio-activity map, not an AVL dashboard.
